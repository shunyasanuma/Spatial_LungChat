...
generate_de_volcano_plot <- function(obj_list,
  ...
  if (!is.null(celltype_filter)) {
    sel1 <- which(meta1[[annotation]] == celltype_filter)
    sel2 <- which(meta2[[annotation]] == celltype_filter)
    message(sprintf("Subsetting to cell type '%s' — %d in %s, %d in %s",
                    celltype_filter, length(sel1), group_1, length(sel2), group_2))
    mat1 <- mat1[, sel1, drop = FALSE]
    mat2 <- mat2[, sel2, drop = FALSE]
  }
  ...
  # Step 4: Convert to clean data.frame and mutate
  ...
  # Step 5: Sort and slice top table and labels
  deg_result <- deg_result[order(-deg_result$log10_padj, -abs(deg_result$avg_log2FC)), ]
  top_table <- head(deg_result, top_n)
  label_genes <- head(deg_result$Gene, label_top_n)
  ...
  volcano <- ggplot(deg_result, aes(x = avg_log2FC, y = log10_padj, color = Sig)) +
    ...
    labs(
      title = paste(group_1, "vs", group_2, if (!is.null(celltype_filter)) paste0(" — ", celltype_filter) else ""),
...
# --------------------------------------------------------------------------
# get_toppcell_enrichment: Enrichment analysis using the ToppCell database
#
# Description:
#   Takes a list of gene symbols, converts them to Entrez IDs using ToppGene,
#   and submits them to the ToppCell API for enrichment. Returns the top 5
#   enriched cell type results based on FDR-corrected p-values.
#
# Parameters:
#   gene_symbols (character): Vector of HGNC gene symbols.
#
# Returns:
#   A data.frame of the top enrichment results (or NULL if failed).
# --------------------------------------------------------------------------
...
get_toppcell_enrichment <- function(gene_symbols) {
  ...
  message("1. Converting gene symbols to Entrez IDs...")
  lookup_url <- "https://toppgene.cchmc.org/API/lookup"
  lookup_body <- list(Symbols = gene_symbols)
  
  lookup_response <- POST(
    url = lookup_url,
    body = toJSON(lookup_body, auto_unbox = TRUE),
    add_headers("Content-Type" = "text/json")
  )
  
  if (status_code(lookup_response) != 200) {
    warning(paste("ToppGene LOOKUP failed. Status:", status_code(lookup_response)))
    return(NULL)
  }
  ...
  if (length(entrez_ids) < 5) {
    warning("Fewer than 5 valid Entrez IDs found. ToppGene requires at least 5.")
    return(NULL)
  }
  
  message(sprintf("   Found %d valid Entrez IDs.", length(entrez_ids)))
  
  # === Step 2: Submit to ToppCell for Enrichment ===
  message("2. Submitting gene list to ToppCell for enrichment...")
  enrich_url <- "https://toppgene.cchmc.org/API/enrich"
  enrich_body <- list(
    Genes = entrez_ids,
    Categories = list(
      list(Type = "ToppCell", PValue = 0.05, MaxResults = 5, Correction = "FDR")
    )
  )
  
  enrich_response <- POST(
    url = enrich_url,
    body = toJSON(enrich_body, auto_unbox = TRUE),
    add_headers("Content-Type" = "text/json")
  )
  
  if (status_code(enrich_response) != 200) {
    warning(paste("ToppGene ENRICH failed. Status:", status_code(enrich_response)))
    return(NULL)
  }
  
  enrich_results <- fromJSON(content(enrich_response, "text", encoding = "UTF-8"))
  
  # === Step 3: Extract and Return Enrichment Results ===
  if (!is.null(enrich_results$Annotations) && length(enrich_results$Annotations) > 0) {
    ...
    if (nrow(results_df) > 0) {
      message("3. Success! Enrichment results received.")
      return(results_df)
    }
  }
  
  message("3. Analysis complete, but no significant enrichment results were found in ToppCell.")
  return(NULL)
}
